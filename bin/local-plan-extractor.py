...
class LocalPlanHousingExtractor:
    ...
    def find_relevant_pages(self, pdf_path: str, top_n: int = None) -> List[int]:
        ...
        try:
            with open(pdf_path, "rb") as file:
                ...
                print(
                    f"  Scanning {total_pages} pages for relevant content...",
                    file=sys.stderr,
                )
                ...
                print(
                    f"  Selected top {len(relevant_pages)} pages (scores: {[s for _, s in page_scores[:top_n]]})",
                    file=sys.stderr,
                )
    ...
    def extract_pages_to_pdf(self, pdf_path: str, page_numbers: List[int]) -> bytes:
        ...
        try:
            with open(pdf_path, "rb") as file:
                ...
                print(
                    f"  Extracted PDF size: {len(pdf_bytes):,} bytes", file=sys.stderr
                )
    ...
    def extract_housing_data(
        ...
        if len(pdf_bytes) > 10_000_000:  # ~10MB limit as safety
            print(
                f"  Warning: Extracted PDF still large, reducing to top {max_pages//2} pages",
                file=sys.stderr,
            )
            return self.extract_housing_data(
                pdf_path, authority_name, max_pages=max_pages // 2
            )
...
"""
        ...
        try:
            print(
                f"  Sending {len(relevant_pages)} pages to Claude API...",
                file=sys.stderr,
            )
            ...
            for attempt in range(1, self.max_retries + 1):
                try:
                    ...
                    if attempt < self.max_retries:
                        ...
                        print(
                            f"  Rate limit hit. Waiting {wait_time}s before retry {attempt}/{self.max_retries}...",
                            file=sys.stderr,
                        )
                    ...
                    if "prompt is too long" in error_msg and max_pages > 8:
                        print(
                            f"  Prompt too long, reducing to {max_pages//2} pages and retrying...",
                            file=sys.stderr,
                        )
                        return self.extract_housing_data(
                            pdf_path, authority_name, max_pages=max_pages // 2
            ...
            if "organisation-name" in housing_data:
                housing_data["organisation"] = self.org_matcher.match(
                    housing_data["organisation-name"]
                )

            # Match organisation codes in housing-numbers array
            if "housing-numbers" in housing_data and isinstance(
                housing_data["housing-numbers"], list
            ):
                for org_entry in housing_data["housing-numbers"]:
                    if "organisation-name" in org_entry:
                        org_entry["organisation"] = self.org_matcher.match(
                            org_entry["organisation-name"]
                        )
                ...
                if len(housing_data["housing-numbers"]) > 1:
                    ...
                    if org_codes:
                        ...
                        if ref_parts:
                            ref_parts.sort()
                            joint_ref = "joint-planning-authority:" + "-".join(
                                ref_parts
                            )
                            ...
                            joint_entry = {
                                "organisation-name": housing_data.get(
                                    "organisation-name", ""
                                ),
                                ...
                                "broad-locations-housing": sums[
                                    "broad-locations-housing"
                                ],
                                "annual-required-housing": sums[
                                    "annual-required-housing"
                                ],
            ...
            if "organisations" in housing_data:
                ...
                for org_entry in housing_data.get("housing-numbers", []):
                    ...
                    if org_name:
                        lpa_code = self.org_matcher.get_local_planning_authority(
                            org_name
                        )
                    ...
                    for entry in housing_data["housing-numbers"]:
                        ...
                        if isinstance(required, (int, float)) and required > 0:
                            if annual == "" or annual is None:
                                ...
                                print(
                                    f"    Calculated annual-required-housing: {calculated_annual} ({required}/{plan_years} years)",
                                    file=sys.stderr,
                                )
    ...
    def extract_from_multiple_pdfs(
        ...
        print(
            f"Rate limit settings: {self.rate_limit_delay}s between API calls, {delay_between_files}s between files\n"
        )
        ...
        for i, pdf_path in enumerate(pdf_files, 1):
            ...
            if "error" not in result:
                ...
                print(
                    f"    Plan period: {result.get('period-start-date', '?')} - {result.get('period-end-date', '?')}"
                )
                ...
                if result.get("housing-numbers") and len(result["housing-numbers"]) > 0:
                    if len(result["housing-numbers"]) == 1:
                        ...
                        print(
                            f"    Required housing: {housing.get('required-housing', 'N/A')}"
                        )
                        print(
                            f"    Allocated housing: {housing.get('allocated-housing', 'N/A')}"
                        )
                        print(
                            f"    Windfall housing: {housing.get('windfall-housing', 'N/A')}"
                        )
                        print(
                            f"    Committed housing: {housing.get('committed-housing', 'N/A')}"
                        )
                        print(
                            f"    Broad locations: {housing.get('broad-locations-housing', 'N/A')}"
                        )
                        ...
                        for org in result["housing-numbers"]:
                            ...
                            if org_code:
                                ...
                                print(f"    • {org_name}:")
                            print(
                                f"      Required: {org.get('required-housing', 'N/A')}"
                            )
                            print(
                                f"      Allocated: {org.get('allocated-housing', 'N/A')}"
                            )
                            print(
                                f"      Windfall: {org.get('windfall-housing', 'N/A')}"
                            )
                            print(
                                f"      Committed: {org.get('committed-housing', 'N/A')}"
                            )
                            print(
                                f"      Broad locations: {org.get('broad-locations-housing', 'N/A')}"
                            )
                            if org.get("pages"):
...
def extract_batch_pdfs(directory: str, api_key: str, delay_between_files: int = 3):
    ...
    results = extractor.extract_from_multiple_pdfs(
        directory, "housing_data.csv", delay_between_files
    )
    ...
    if successful:
        total_reqs = [
            r["required-housing"] for r in successful if r.get("required-housing")
        ]
        if total_reqs:
  ...
  python script.py ./local_plans/ --api-delay 5 --file-delay 10
    ...
    if path.is_file():
        ...
        if args.output:
            ...
            print(
                f"Successfully processed: {len(successful)}/{len(results)}",
                file=sys.stderr,
            )
            ...
            if successful:
                ...
                if total_reqs:
                    ...
                    print(
                        f"Average requirement: {sum(total_reqs)/len(total_reqs):,.0f} homes",
                        file=sys.stderr,
                    )
                    print(
                        f"Total homes across all plans: {sum(total_reqs):,.0f}",
                        file=sys.stderr,
                    )
        else:
